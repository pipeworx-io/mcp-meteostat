...
`tools/list` at `https://gateway.pipeworx.io/meteostat/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/meteostat_find_stations \
  -H 'Content-Type: application/json' \
  -d '{"query":"San Francisco","country":"US"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/meteostat_find_stations`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
